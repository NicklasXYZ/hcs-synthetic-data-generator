    ...
    if (normalizedPath.endsWith("/vis")) {
      ...
      Promise.all([
        ...
        .then(([patients, practitioners, events]) => {
          // Slice the practitioners array to include only the first 5 practitioners
          const sampledPractitioners = practitioners.slice(0, 5);
  
          // Create lookup maps
          const patientMap = new Map(patients.map((p) => [p.id, p]));
          const practitionerMap = new Map(sampledPractitioners.map((p) => [p.id, p]));
  
          // Create a set of sampled patient IDs
          const sampledPatientIds = new Set();
  
          // Filter patients to include only the first 10 patients per practitioner
          sampledPractitioners.forEach((practitioner) => {
            const practitionerPatients = patients.filter((patient) =>
              events.some((event) => event.practitioner_id === practitioner.id && event.patient_id === patient.id)
            );
            practitionerPatients.slice(0, 10).forEach((patient) => {
              sampledPatientIds.add(patient.id);
            });
          });
  
          // Filter events to include only those related to sampled patients and practitioners
          const filteredEvents = events.filter(
            (e) =>
              sampledPatientIds.has(e.patient_id) &&
              practitionerMap.has(e.practitioner_id)
          );
  
          // Store data with valid references only
          window.vizData = {
            patientMap: new Map([...patientMap].filter(([id]) => sampledPatientIds.has(id))),
            practitionerMap,
            events: filteredEvents,
            practitioners: sampledPractitioners,
          };
      ...
      `;
          ...
          const firstPractitionerId = sampledPractitioners.sort((a, b) =>
